...
if($stmt->num_rows>0)
{
	...
	while($stmt->fetch())
	{
		...
		$screen['sequence'] = $screen_sequence;
		$screen['layout']=array();
		...
		while($stmt1->fetch())
		{
			...
			$layout['orientation'] = $layout_orientation;
			$layout['attribute'] = $layout_attribute;
			$layout['sequence'] = $layout_sequence;
			$layout['component']=array();
			
			$stmt2 = $con->prepare("SELECT a.id, a.name, a.type, a.bg_image, a.image, a.save_value, a.mandatory, a.options, a.default_value, a.attribute, a.onclick_function, a.onclick_target, a.onclick_target_value_id, a.sequence FROM $table3 a WHERE a.screen_id='".$screen['id']."' AND a.layout_id='".$layout['id']."' AND a.status=1 ORDER BY a.sequence") or die($con->error);
			$stmt2->execute();
			$stmt2->store_result();
			...
			while($stmt2->fetch())
			{
				...
				$component['save_value']=$component_save_value;
				$component['mandatory']=$component_mandatory;
				$component['options']=$component_options;
				$component['default_value']=$component_default_value;
				$component['attribute']=$component_attribute;
