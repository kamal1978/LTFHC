			...
			<div>
				...
				<div class="table-responsive">
					...
					<?php
						...
						$stmt = $con->prepare("SELECT a.id, a.name, a.type, a.save_value, a.options, a.default_value, a.attribute, a.onclick_function, a.onclick_target, d.name, b.name, c.name, a.sequence, a.status FROM $table1 a LEFT JOIN $table2 b ON a.layout_id=b.id AND b.status=1 LEFT JOIN $table3 c ON a.screen_id=c.id LEFT JOIN $table3 d ON a.onclick_target_value_id=d.id AND c.status=1 WHERE a.id!='' $where ORDER BY c.sequence, a.status, b.sequence, a.sequence LIMIT $start,$limit") or die($con->error);
						$stmt->execute();
						//$result = $stmt->get_result();
						$stmt->bind_result($id, $component, $type, $save_value, $options, $default_value, $attribute, $onclick_function, $onclick_target, $onclick_target_value, $layout_name, $screen_name, $sequence, $status);
						$i=1;
						while ($stmt->fetch())
						{
							switch($status)
