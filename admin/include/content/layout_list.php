        ...
        xmlhttp.send("status="+status_change);
			...
			<div>
				...
				<div>
					...
					<?php
						$stmt = $con->prepare("SELECT a.id, a.name, a.type, a.orientation, a.attribute, c.name, b.name, a.screen_id, a.sequence, a.status FROM $table1 a LEFT JOIN $table2 b ON a.screen_id=b.id LEFT JOIN $table1 c ON a.parent_layout_id=c.id WHERE a.id!='' $where ORDER BY b.sequence, a.sequence LIMIT $start,$limit") or die($con->error);
						$stmt->execute();
